...
internal object BrokerageNoteValidator {
    ...
    internal fun validate(note: BrokerageNote) {
        ...
        if (note.assets.isEmpty()) throw IllegalArgumentException("assets must not be empty")
        ...
        if (summary.totalVolumeTraded <= 0) throw IllegalArgumentException("total volume must be > 0")
        ...
        if (buysCents != expectedBuysCents || sellsCents != expectedSellsCents) {
            ...
            val computed = if (buysCents != expectedBuysCents) buysCents.toDollars() else sellsCents.toDollars()
            throw IllegalArgumentException("buys/sells totals mismatch: expected $expected, got $computed")
        }
        ...
        if (note.assets.size != note.assets.toSet().size) {
            ...
            note.assets.forEachIndexed { index, asset ->
                if (!seen.add(asset)) {
                    ...
                    throw IllegalArgumentException("duplicate asset detected: ${asset.ticker} at index $index is identical to asset at index $firstIndex")
                }
            }
        }
    }
