...
import com.eferraz.pokedex.entities.ProfitabilityIndex.INFLATION_LINKED
import com.eferraz.pokedex.entities.ProfitabilityIndex.POST_FIXED
import com.eferraz.pokedex.entities.ProfitabilityIndex.PRE_FIXED
 ...
 */
internal data class FixedIncomeView(
    val id: String,
    val owner: String,
    ...
    constructor(it: FixedIncome) : this(
        id = it.id.toString(),
        owner = it.owner.name,
        brokerageFirm = it.brokerageFirm.name,
        investedAmount = it.investedAmount.currencyFormat(),
        purchaseDate = it.purchaseDate.format(Format { day(); char('/'); monthNumber(); char('/'); year() }),
        maturityDate = it.maturityDate.format(Format { day(); char('/'); monthNumber(); char('/'); year() }),
        profitability = it.profitability.toString() + "%",
        profitabilityIndex = when (it.profitabilityIndex) {
            POST_FIXED -> "CDI"
            PRE_FIXED -> "Prefixado"
            INFLATION_LINKED -> "IPCA"
        },
        type = it.type.name,
        liquidity = when (it.liquidity) {
            Liquidity.DAILY -> "Diário"
            Liquidity.AT_MATURITY -> "No Venc."
        },
        issuerBank = it.issuerBank.name,
        description = it.description,
        currentAmount = it.currentAmount.currencyFormat(),
