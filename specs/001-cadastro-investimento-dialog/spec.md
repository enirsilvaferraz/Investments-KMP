...
** — **evitar** *factories* de teste (ex. `TestDataFactory`) para código novo, **testes obrigatórios** para mudanças em `core/domain/usecases/` (módulo
Gradle **`:domain:usecases`**) — princípio V), **consistência de UX** (formatação, erros; **pré-visualizações `@Preview` no mesmo ficheiro que o
composable** — princípio VI), **desempenho** (latência, volumes) e **coerência** código ↔ documentação ↔ `.specify` ↔ `.cursor` (princípios IV–IX);
